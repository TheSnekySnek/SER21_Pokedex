...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:key name="type" match="/pokedex/pokemon/type/text()" use="." /> <!-- Utilisé pour la récupération des types -->
			...
			<body>
				...
				<div class="container-fluid">
					...
					<div id="accordion">
						...
						<xsl:variable name="types" select="/pokedex/pokemon/type/text()[generate-id() = generate-id(key('type',.)[1])]" /> <!-- ##### A compléter 3 : Ici, vous devez trouver l'expression XPath à mettre dans l'attribut select 
	...
	<xsl:template name="lister_pokemon">
		...
		<div class="row">
			...
			<xsl:for-each select="$filtre">
				...
				<xsl:sort order="ascending" select="id" data-type="number"/> <!-- ##### A compléter 7 : Vous devez trier les pokemons par la valeur numérique de leur ID -->
